...
import type {
  ...
  CreateEntryCommand,
  StartingBalanceDTO, // Placeholder if needed, but not used here
} from "../../../types";
...
export const GET: APIRoute = async ({ locals, request }) => {
  ...
  try {
    ...
    if (!user) {
      console.log(
        `[WARN] [${requestId}] Unauthorized access attempt to GET entries`,
      );
    ...
    const { userId } = user;
    console.log(
      `[INFO] [${requestId}] Fetching entries list for user ${userId}`,
    );
    ...
    console.log(
      `[INFO] [${requestId}] Successfully retrieved ${entriesList.data.length} entries for user ${userId}`,
    );
    ...
    console.error(
      `[ERROR] [${requestId}] Error retrieving entries list:`,
      error,
    );
...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    if (!user) {
      console.log(
        `[WARN] [${requestId}] Unauthorized access attempt to POST entry`,
      );
      const error = createUnauthorizedError();
      return new Response(JSON.stringify(error.body), {
        status: error.status,
    ...
    console.log(
      `[INFO] [${requestId}] Successfully created entry ${createdEntry.id} for user ${userId}`,
    );
